...
def create_network_graph(current_state):
    ...
    nodes = [
        {"id": "anonymize_question", "label": "匿名化问题", "x": 0, "y": 0},
        {"id": "planner", "label": "制定计划", "x": 175*1.75, "y": -100},
        {"id": "de_anonymize_plan", "label": "还原计划", "x": 350*1.75, "y": -100},
        {"id": "break_down_plan", "label": "拆解子任务", "x": 525*1.75, "y": -100},
        {"id": "task_handler", "label": "任务路由", "x": 700*1.75, "y": 0},
        {"id": "retrieve_chunks", "label": "检索正文切片", "x": 875*1.75, "y": +200},
        {"id": "retrieve_summaries", "label": "检索章节摘要", "x": 875*1.75, "y": +100},
        {"id": "retrieve_book_quotes", "label": "检索财务指标", "x": 875*1.75, "y": 0},
        {"id": "answer", "label": "上下文回答", "x": 875*1.75, "y": -100},
        {"id": "replan", "label": "重新规划", "x": 1050*1.75, "y": 0},
        {"id": "can_be_answered_already", "label": "判断可否回答", "x": 1225*1.75, "y": 0},
        {"id": "get_final_answer", "label": "生成最终答案", "x": 1400*1.75, "y": 0}
    ]
...
def main():
    ...
    st.title("智能财报研读系统 (Agentic 10-K Copilot) — Powered by LangGraph")
    
    # Load your existing agent creation function
    plan_and_execute_app = create_agent()

    # Get the user's question
    question = st.text_input("请输入您关于苹果10-K财报的问题:", "2025财年，苹果的服务(Services)业务收入表现如何？管理层在财报中给出了哪些主要驱动因素？")

    if st.button("开始分析 Run Agent"):
        inputs = {"question": question}
        
        # Create a row for the graph
        st.markdown("**Agent 执行流程图**")
        ...
        with col1:
            st.markdown("**执行计划 Plan**")
        with col2:
            st.markdown("**已完成步骤 Past Steps**")
        with col3:
            st.markdown("**聚合上下文 Aggregated Context**")
